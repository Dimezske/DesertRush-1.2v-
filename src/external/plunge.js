...
export default class Plunge extends Phaser.Physics.Arcade.Sprite {
      ...
      function () {
        if (this.scene.player.anims.isPlaying == true) {
          ...
          if (this.x < this.scene.player.x) {
            ...
            this.dtext = this.scene.add
              .text(this.x, this.y, `${this.scene.player.damage}`)
              .setTint(0xfc4903);
      ...
      function () {
        if (this.scene.schimitar.anims.isPlaying == true) {
          ...
          if (this.x < this.scene.player.x) {
            ...
            this.dtext = this.scene.add
              .text(this.x, this.y, `${this.scene.schimitar.damage}`)
              .setTint(0xfc4903);
            //this.scene.time.delayedCall(1200, () => {this.dtext.destroy();/*this.scene.player.exp+=this.exp*/})
            this.scene.time.addEvent({
              delay: 1200,
            ...
            this.setPosition(this.x + 150, this.y);
            this.dtext = this.scene.add
              .text(this.x, this.y, `${this.scene.player_punch.damage}`)
              .setTint(0xfc4903);
            this.dtext = this.scene.add
              .text(this.x, this.y, `${this.scene.schimitar.damage}`)
              .setTint(0xfc4903);
  ...
  monster__ani_setup() {
    this.anims.create({
      key: "idle",
      frames: this.anims.generateFrameNumbers("plunge-sprite", {
        start: 2,
        end: 2,
      }),
      frameRate: 15,
  ...
  checkMonsterMovement() {
    ...
    if (this.countdown <= 0) {
      // this will trigger every this.time seconds
      let fps = 60;
  ...
  setVertikalMovement() {
    if (this.body.velocity.y <= 0) {
      // if we are going in -Y direction go in +Y now
  ...
  setHorizontalMovement() {
    if (this.body.velocity.x <= 0) {
      // if we are going in -X direction go in +X now
  ...
  setRandomXMovement() {
    // random movement in X direction
    const randomLeftRight = Math.random() <= 0.5 ? 1 : -1;
    this.body.setVelocityX(this.getRandomAround(this.speed) * randomLeftRight);
  }
  setRandomYMovement() {
    // random movement in Y direction
    const randomUpDown = Math.random() <= 0.5 ? 1 : -1;
    this.body.setVelocityY(this.getRandomAround(this.speed) * randomUpDown);
  }
  getRandomAround(val) {
