...
export default class Player extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y, key, frame) {
    ...
    this.keys = {
      ...
      runAndCling: this.scene.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SHIFT),
  ...
  control_handler() {
    ...
    //----------------Clinging Cliff Hang------------------------------
    if (this.keys["runAndCling"].isDown) {
      console.log("is clinging");
      this.body.setAcceleration(0, 0);
      this.scene.sound.play("cling");
    }
    if (this.keys["duck"].isDown) {
      console.log("is not clinging");
      this.body.setAcceleration(0, 0);
    }
    if (this.keys["runAndCling"].isDown && this.body.touching.left) {
      this.isClinging = true;
      this.body.setVelocityY(0, 0),
        this.body.setVelocityX(0, 0),
        this.anims.play("cling-left");
      if (this.isClinging) {
        if (this.keys["walkLeft"].isDown && this.keys["runAndCling"].isDown) {
          this.body.setVelocityX(-160);
          this.anims.play("left", false);
          this.anims.play("cling-left", true);
          this.scene.sound.play("cling");
        }
        if (this.keys["runAndCling"].isDown && this.keys["jump"].isDown) {
          this.body.setVelocityY(-50);
        }
      }
    }
    if (this.keys["runAndCling"].isDown && this.body.touching.right) {
      this.isClinging = true;
      this.body.setVelocityY(0, 0),
        this.body.setVelocityX(0, 0),
        this.anims.play("cling-left");
      if (this.isClinging) {
        if (this.keys["walkRight"].isDown && this.keys["runAndCling"].isDown) {
          this.body.setVelocityX(160);
