...
this.scene.physics.add.collider(this, this.scene.schimitar, () => {
    ...
    if (this.health <= 0) {
      ...
      this.setFrame(4);
    } else if (this.health < 16.0) {
      this.setFrame(3);
    } else if (this.health < 25.0) {
      this.setFrame(2);
    

    }else  if (this.health < 30.0) {
      this.setFrame(1);
    }else if (this.health < 35.0) {
      this.setTexture("crate-explode");
      this.setFrame(0);
    }else  if (this.health == 50.0&&this.loot==true) {
      //this.scene.addCarrotAbove(this,0xfc8c03);
      this.scene.dropSchimitar(this);
      this.setTexture("crate-open");
this.loot=false;
      this.x += 40;
      this.y -= 55;
    }else  if (this.health < 65.0) {
      this.setFrame(2);
    }else   if (this.health < 70.0) {
      this.setFrame(1);
    }else  if (this.health <= 75.0) {
      this.setFrame(0);
    }
  }
}